...
일반 PDF는 Vault의 `Paper/`에 넣으면 Obsidian 논문 목록과 그래프에 표시됩니다. **분석은 각 파일 옆의 `분석` 버튼을 누른 경우에만 요청됩니다.** 기존 `Inbox/` 자동 접수 경로는 호환용이며 새 PDF 분석을 자동으로 시작하는 기본 경로가 아닙니다.
...
`publish`는 로컬 Vault 결과를 만들고 같은 원본의 `Paper/` 경로를 `Dashboard/pdf-links.json`에 연결합니다. Git push와 원격 receipt 검증은 별도 단계입니다. 자세한 수동 연결 절차와 모델 단계는 `CHAT_AUTOMATION.md`에 있습니다. 원문·생성 이미지의 사용 및 공유 권한은 사용자 책임 범위에서 확인하세요.
