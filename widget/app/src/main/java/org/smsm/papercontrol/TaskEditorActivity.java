...
public class TaskEditorActivity extends Activity {
 ...
 private String action,key,raw;private boolean writing,confirmDelete,meeting;private String meetingDay,meetingTime,draftId;private Button dayButton,timeButton;
 ...
 @Override public void onCreate(Bundle state){super.onCreate(state);action=getIntent().getStringExtra("action");key=getIntent().getStringExtra("key");raw=getIntent().getStringExtra("raw");if(!"add".equals(action)&&!"edit".equals(action)){finish();return;}
  meeting=state!=null?state.getBoolean("meeting"):MeetingDraft.isMeeting(action,getIntent().getStringExtra("entry_kind"),PaperWidget.meetings(this,getIntent().getIntExtra("widget",0)));
  meetingDay=state!=null?state.getString("day"):VaultStore.day();meetingTime=state!=null?state.getString("time"):"09:00";draftId=state!=null?state.getString("draftId"):java.util.UUID.randomUUID().toString();
  ...
  heading.addView(text(meeting?"회의 일정 추가":("add".equals(action)?"할 일 추가":"할 일 수정"),21,"#20372D",true),new LinearLayout.LayoutParams(0,dp(48),1));
  ...
  TextView subtitle=text(meeting?"날짜와 시간을 정해 회의 목록에 담아 두세요.":("add".equals(action)?"오늘 목록에 바로 담아 두세요.":"내용을 바꿔도 일정과 기록은 유지됩니다."),13,"#63736A",false);LinearLayout.LayoutParams sub=new LinearLayout.LayoutParams(-1,-2);sub.bottomMargin=dp(18);card.addView(subtitle,sub);
  input=new EditText(this);input.setId(android.R.id.edit);input.setTextSize(17);input.setMinLines(2);input.setMaxLines(3);input.setInputType(android.text.InputType.TYPE_CLASS_TEXT|android.text.InputType.TYPE_TEXT_FLAG_CAP_SENTENCES);input.setHorizontallyScrolling(false);input.setGravity(Gravity.TOP|Gravity.START);input.setHint(meeting?"어떤 회의인가요?":"무엇을 할까요?");input.setHintTextColor(Color.parseColor("#68776E"));input.setTextColor(Color.parseColor("#20372D"));input.setPadding(dp(15),dp(15),dp(15),dp(15));input.setBackground(surface("#FFFFFF","#CAD8CF",12));input.setImeOptions(android.view.inputmethod.EditorInfo.IME_ACTION_DONE);card.addView(input,new LinearLayout.LayoutParams(-1,dp(94)));
  if("edit".equals(action)){try{input.setText(new TaskDocument(raw).tasks.get(0).editable);}catch(Exception e){finish();return;}}
  if(state!=null)input.setText(state.getString("draftTitle",""));
  if(meeting){LinearLayout when=new LinearLayout(this);LinearLayout.LayoutParams wp=new LinearLayout.LayoutParams(-1,dp(50));wp.topMargin=dp(12);card.addView(when,wp);dayButton=button(meetingDay,false);timeButton=button(meetingTime,false);LinearLayout.LayoutParams left=new LinearLayout.LayoutParams(0,-1,1.35f);left.setMarginEnd(dp(8));when.addView(dayButton,left);when.addView(timeButton,new LinearLayout.LayoutParams(0,-1,1));
    dayButton.setContentDescription("회의 날짜 "+meetingDay);timeButton.setContentDescription("회의 시간 "+meetingTime);
    dayButton.setOnClickListener(v->{java.time.LocalDate d=java.time.LocalDate.parse(meetingDay);new android.app.DatePickerDialog(this,(picker,y,m,day)->{meetingDay=java.time.LocalDate.of(y,m+1,day).toString();dayButton.setText(meetingDay);dayButton.setContentDescription("회의 날짜 "+meetingDay);},d.getYear(),d.getMonthValue()-1,d.getDayOfMonth()).show();});
    timeButton.setOnClickListener(v->{java.time.LocalTime t=java.time.LocalTime.parse(meetingTime);new android.app.TimePickerDialog(this,(picker,h,m)->{meetingTime=String.format(java.util.Locale.ROOT,"%02d:%02d",h,m);timeButton.setText(meetingTime);timeButton.setContentDescription("회의 시간 "+meetingTime);},t.getHour(),t.getMinute(),true).show();});
  }
 ...
 @Override protected void onSaveInstanceState(Bundle state){super.onSaveInstanceState(state);state.putBoolean("meeting",meeting);state.putString("day",meetingDay);state.putString("time",meetingTime);state.putString("draftId",draftId);state.putString("draftTitle",input.getText().toString());}
 ...
 private void save(){if(writing)return;try{persist(action,meeting?MeetingDraft.title(input.getText().toString()):TaskDocument.cleanTitle(input.getText().toString()));}catch(Exception e){error.setText(e.getMessage());error.setVisibility(View.VISIBLE);input.setBackground(surface("#FFFFFF","#B86B62",12));}}
 private void persist(String verb,String title){if(writing)return;writing=true;save.setEnabled(false);save.setText("저장 중…");close.setEnabled(false);if(remove!=null)remove.setEnabled(false);input.setEnabled(false);if(meeting){dayButton.setEnabled(false);timeButton.setEnabled(false);}new Thread(()->{try{if(meeting)new VaultStore(this).addMeeting(title,meetingDay,meetingTime,draftId);else new VaultStore(this).change(verb,key,raw,title);PaperWidget.updateAll(this);runOnUiThread(this::finish);}catch(Exception e){runOnUiThread(()->{writing=false;save.setEnabled(true);save.setText("add".equals(action)?"추가하기":"저장하기");close.setEnabled(true);input.setEnabled(true);if(meeting){dayButton.setEnabled(true);timeButton.setEnabled(true);}if(remove!=null)remove.setEnabled(true);error.setText(e.getMessage()==null?"저장하지 못했습니다.":e.getMessage());error.setVisibility(View.VISIBLE);});}},"floating-task-editor").start();}
}
