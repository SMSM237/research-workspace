...
public final class VaultStore {
  ...
  public void addMeeting(String title,String day,String time,String id)throws Exception{synchronized(WRITE_LOCK){
    MeetingDraft draft=new MeetingDraft(title,day,time,id);Uri existing=resolve(draft.path,false,false);
    if(existing!=null){if(readUri(existing).equals(draft.text))return;throw new IOException("같은 회의 저장 위치가 이미 사용 중입니다. 다시 열어 주세요.");}
    writeNew(draft.path,draft.text);
    try{String activity=".research-activity/"+day()+"/"+TaskDocument.hash(draft.path)+".json";if(resolve(activity,false,false)==null)writeNew(activity,new JSONObject().put("day",day()).put("path",draft.path).toString());}catch(Exception ignored){}
    prefs(context).edit().putString("notice","회의 저장됨 · 동기화 전").apply();
  }}
