...
export interface RunRequest {version:1;id:string;action:'analyze-inbox'|'diagnostic';createdAt:string;maxPapers:number;}
...
export const STATES:Record<string,string>={pending:'PC 접수 대기',dispatching:'실행 요청 전달 중',queued:'Codex 실행 대기',running:'분석 중',waiting:'확인 대기',blocked:'조치 필요',complete:'Git 게시 완료',verified:'연결 확인 완료',empty:'새 논문 없음',cancelled:'요청 취소'};
export function parseRequest(text:string,filename?:string):RunRequest {
  if(text.length>2048)throw Error('실행 요청이 너무 큽니다.');
  const r=JSON.parse(text);
  if(!r||typeof r!=='object'||Array.isArray(r)||Object.keys(r).sort().join(',')!=='action,createdAt,id,maxPapers,version'||r.version!==1||!UUID.test(r.id)||!['analyze-inbox','diagnostic'].includes(r.action)||!Number.isInteger(r.maxPapers)||r.maxPapers<1||r.maxPapers>10||typeof r.createdAt!=='string'||!Number.isFinite(Date.parse(r.createdAt)))throw Error('실행 요청 형식이 올바르지 않습니다.');
...
export function queuePrompt(id:string,diagnostic:boolean,runbook:string):string {
