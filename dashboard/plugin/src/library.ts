...
export class PaperLibrary extends ItemView {
  ...
  renderList(){const list=this.contentEl.querySelector<HTMLElement>('.rr-library-list');if(!list)return;list.empty();const match=(s:string)=>s.toLocaleLowerCase().includes(this.query.toLocaleLowerCase());const control=(this.plugin as any).remoteControl;
    ...
    if(pdfs.length)list.createEl('h3',{text:'원본 PDF',cls:'rr-library-section'});
    for(const file of pdfs){const row=list.createDiv({cls:'rr-library-pdf-row'});const open=row.createEl('button',{cls:'rr-library-paper',text:file.path.startsWith('Sources/')?`${file.path.split('/')[1]} · 원본 PDF`:file.basename,attr:{type:'button','aria-label':file.basename+' 원본 PDF 열기'}});open.onclick=async()=>{await this.app.workspace.getLeaf(false).openFile(file);if(Platform.isMobile)this.app.workspace.leftSplit.collapse();};if(file.path.startsWith('PDF/')){const analyze=row.createEl('button',{cls:'rr-library-analyze',text:'분석',attr:{type:'button','aria-label':file.basename+' 분석 시작'}});analyze.onclick=()=>{analyze.disabled=true;void control.submitPdf(file).catch(()=>{}).finally(()=>{analyze.disabled=false;this.renderList();});};}}
    let count=0;for(const file of papers){const fm=this.app.metadataCache.getFileCache(file)?.frontmatter;const title=String(fm?.library_title||file.basename);if(!match(title))continue;if(!count)list.createEl('h3',{text:'분석 리포트',cls:'rr-library-section'});count++;const b=list.createEl('button',{cls:'rr-library-paper',text:title});b.title=title;b.onclick=async()=>{await this.app.workspace.getLeaf(false).openFile(file,{state:{mode:'preview'}});if(Platform.isMobile)this.app.workspace.leftSplit.collapse();};}
    if(!count&&!pdfs.length)list.createEl('p',{text:this.query?'검색 결과가 없습니다.':'PDF 폴더에 파일을 넣으면 여기에 표시됩니다.',cls:'rr-empty'});
    void control?.requests().then((requests:any[])=>{for(const row of Array.from(list.querySelectorAll<HTMLElement>('.rr-library-pdf-row'))){const file=pdfs[Array.from(list.querySelectorAll('.rr-library-pdf-row')).indexOf(row)];const request=requests.find(x=>x.request.version===2&&x.request.path===file.path);const b=row.querySelector<HTMLButtonElement>('.rr-library-analyze');if(b&&request){b.textContent=({pending:'전송 대기',queued:'접수 대기',running:'분석 중',review:'검증 중',publishing:'게시 중',waiting:'확인 대기',blocked:'조치 필요',complete:'완료'} as Record<string,string>)[request.status.state]||'상태 보기';b.onclick=()=>void control.open();}}}).catch(()=>{});
  }
}
