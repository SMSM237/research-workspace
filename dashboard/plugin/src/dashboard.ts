...
class ModuleView extends MarkdownRenderChild {
  ...
  onload(){this.alive=true;const saved=this.dashboard.app.loadLocalStorage('research-dashboard-view')||{};if(typeof saved.project==='string')this.projectPath=saved.project;const root=this.containerEl;root.empty();root.classList.add('rd-module');root.dataset.module=this.kind;
   ...
   private setupCollapse(heading:HTMLElement,root:HTMLElement,kind:'projects'|'meetings',collapsed:boolean){
    ...
    const apply=(value:boolean)=>{root.classList.toggle('is-collapsed',value);root.closest('.rd-desktop-right')?.classList.toggle(`is-${kind}-collapsed`,value);root.closest('.rd-desktop-grid')?.classList.toggle(`is-${kind}-collapsed`,value);toggle.setAttribute('aria-expanded',String(!value));toggle.setAttribute('aria-label',label+(value?' 펼치기':' 접기'));toggle.title=label+(value?' 펼치기':' 접기');setIcon(toggle,value?'chevron-down':'chevron-up');};
    apply(collapsed);toggle.onclick=()=>{const next=!root.classList.contains('is-collapsed');apply(next);const current=this.dashboard.app.loadLocalStorage('research-dashboard-view')||{};this.dashboard.app.saveLocalStorage('research-dashboard-view',{...current,[key]:next});};
   }
  ...
  private async render(){const ticket=++this.ticket;try{const s=await this.dashboard.snapshot();if(!this.alive||ticket!==this.ticket)return;const count=this.containerEl.querySelector<HTMLElement>('.rd-card-count');if(count)count.textContent=String(this.kind==='queue'?s.pdfs.filter(f=>!s.pdfLinks[f.path]).length:this.kind==='papers'?s.papers.length:s.meetings.length);const previousScroll=this.body.scrollTop;this.graph3d?.destroy();this.graph3d=null;this.body.empty();
    if(this.kind==='tasks'){
      ...
      if(!pdfs.length)this.empty('대기 중인 원본 PDF가 없습니다. Paper 폴더에 넣으면 여기에 나타납니다.');
  ...
  private renderGraph(s:Snapshot,target=this.body,expanded=false){
    ...
    if(!items.length){target.createEl('p',{cls:'rd-empty',text:'Paper 폴더에 파일을 넣으면 그래프에 나타납니다.'});return;}
  ...
  private async renderHome(){const ticket=++this.ticket;try{const counts=await this.dashboard.extras.activity();if(!this.alive||ticket!==this.ticket)return;this.body.empty();const left=this.body.createDiv({cls:'rd-greeting'});const today=new Date();left.createEl('h1',{text:today.toLocaleDateString('ko-KR',{month:'long',day:'numeric',weekday:'long'})});const weather=left.createDiv({cls:'rd-weather',text:'서울 · 날씨 확인 중'});void this.dashboard.extras.weather().then(text=>{if(weather.isConnected)weather.textContent=text;});const controls=left.createDiv({cls:'rd-weather-actions'});this.button(controls,'현재 위치',async()=>{weather.textContent=await this.dashboard.extras.useLocation();});this.button(controls,'서울',async()=>{weather.textContent=await this.dashboard.extras.seoul();});const source=controls.createEl('a',{text:'Open-Meteo',href:'https://open-meteo.com/',attr:{target:'_blank',rel:'noopener'}});
    ...
    const fileCount=personalFileCount(this.dashboard.app.vault.getFiles().map(file=>file.path));right.createEl('p',{cls:'rd-personal-file-count',text:`보관 파일 ${fileCount}개`,attr:{title:'Paper · Paper reports · 새로 만든 폴더의 파일 합계. 분석 내부 자료는 제외합니다.'}});
