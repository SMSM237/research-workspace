...
| 구성 | 버전 | 포함 내용 |
|---|---|---|
| 대시보드 / Figure First Reader | 0.7.7 | 개별 PDF 분석 버튼, 대기·완료 목록, 자동 회전하는 3D 그래프, PC·모바일 홈 |
...
리포트의 제목·개념·태그를 기기 안에서 비교하여 공통 주제를 묶습니다. 원본 PDF와 생성된 리포트 사이에는 직접 연결을 표시합니다. 점을 누르면 PC에서는 오른쪽에 원본 또는 리포트를 열고 대시보드를 유지합니다. 모바일에서는 해당 파일을 엽니다. 그래프 구체는 화면이 보이는 동안 자동 회전하고, 7~12초마다 가로·세로 회전 방향을 무작위로 부드럽게 바꿉니다. 휠 확대·축소, 드래그 이동, Shift+드래그 회전이나 키보드 조작 후 잠시 지나면 확대율과 위치가 부드럽게 기본값으로 돌아갑니다. 동작 줄이기 설정에서는 자동 회전하지 않습니다. 색상별 공통 주제 연결은 탐색용 추정이며 인용·인과 관계를 의미하지 않습니다.
...
- 상단 오른쪽에서 그래프를 보고, 분석 대기 원본 PDF와 분석 완료 리포트를 각각의 카드에서 엽니다. 회의록 카드를 접으면 분석 완료 목록이 넓어집니다.
